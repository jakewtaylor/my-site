...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_tinycolor2__ = __webpack_require__(2);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__Css__ = __webpack_require__(3);
...
(function () {
    // Create CSS Helper
    var css = new __WEBPACK_IMPORTED_MODULE_1__Css__["a" /* default */]();

    // Get the album color palette
    var _window = window,
        palette = _window.palette;

    // Convert the colors to tinycolor instances

    var colors = palette.map(function (_ref) {
        var _ref2 = _slicedToArray(_ref, 3),
            r = _ref2[0],
            g = _ref2[1],
            b = _ref2[2];

        return __WEBPACK_IMPORTED_MODULE_0_tinycolor2___default()({ r: r, g: g, b: b });
    });

    // Determine which color to use for the background
    var newBackgroundColor = getNicest(colors);

    // Get the current text color (outside of the card)
    var currentTextColor = __WEBPACK_IMPORTED_MODULE_0_tinycolor2___default()(css.get('contrasting'));

    // If the text won't be readable, switch the text from black to white.
    if (!__WEBPACK_IMPORTED_MODULE_0_tinycolor2___default.a.isReadable(newBackgroundColor, currentTextColor)) {
        css.set('contrasting', css.get('white'));
    }

    if (newBackgroundColor.isDark()) {
        css.set('shadow', __WEBPACK_IMPORTED_MODULE_0_tinycolor2___default()(css.get('white')).setAlpha(0.2).toRgbString());
    }

    // Determine which color to use as the complementary
    var complementary = getComplementary(newBackgroundColor);
...
function getNicest(colors) {
    var sorted = colors.sort(function (a, b) {
        var aSaturation = a.toHsl().s;
        var bSaturation = b.toHsl().s;
...
function getComplementary(color) {
    var complementary = color.complement();

    return complementary.getBrightness() < 30 ? complementary.brighten(50).saturate(50) : complementary;
}
